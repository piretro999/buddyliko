...
class CSVField:
    ...
    def to_schema_field(self) -> Dict:
        ...
        # Parse path - prefer xmlpath over campo
        actual_path = self.xmlpath if self.xmlpath else self.campo
        parts = actual_path.split('/' if '/' in actual_path else '.')
        field_id = actual_path.replace('/', '_').replace('.', '_')
        
        # Determine type from business term or default
        field_type = self._infer_type()
        
        # Parse cardinality
        is_required = self.obbligatorio.upper() in ['SI', 'X']  # X also means required!
        is_array = 'N' in self.numerosità or '*' in self.numerosità
        
        return {
            'id': field_id,
            'name': parts[-1] if len(parts) > 1 else self.campo,
            'path': actual_path,  # Use actual_path (xmlpath or campo)
            'type': field_type,
...
class CSVSchemaParser:
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                field_data = {
                    ...
                    'xmlpath': (row.get('xmlpath') or row.get('lunghezza') or '').strip(),  # Use lunghezza as fallback!
    ...
    def _build_schema_output(self, name: str) -> Dict[str, Any]:
        ...
        # Detect format and root element
        format_type, root_element, namespace = self._detect_metadata()
        
        return {
            'name': name,
            'type': 'csv',
            'format': format_type,  # xml, json, etc.
            'rootElement': root_element,  # Invoice, FatturaElettronica, etc.
            'namespace': namespace,  # Full namespace URI
            'fields': schema_fields,
            'root_fields': root_fields,
            'field_count': len(schema_fields)
        }
    
    def _detect_metadata(self) -> tuple:
        """Detect format, root element, and namespace from fields"""
        # Check first few fields for patterns
        sample_fields = list(self.fields.values())[:10]
        
        # Check for UBL patterns (cbc:, cac:, ext:)
        has_ubl_prefix = any(
            field.campo.startswith(('cbc.', 'cac.', 'ext.')) or
            'cbc:' in (field.xmlpath or '') or
            'cac:' in (field.xmlpath or '')
            for field in sample_fields
        )
        
        # Check for FatturaPA patterns
        has_fatturapa = any(
            'FatturaElettronica' in field.campo or
            'FatturaElettronica' in (field.xmlpath or '')
            for field in sample_fields
        )
        
        if has_ubl_prefix:
            # UBL schema
            # Detect document type from InvoiceTypeCode, CreditNote, etc.
            doc_type = 'Invoice'  # default
            for field in self.fields.values():
                campo_lower = field.campo.lower()
                if 'invoice' in campo_lower and 'credit' not in campo_lower:
                    doc_type = 'Invoice'
                    break
                elif 'creditnote' in campo_lower or 'credit' in campo_lower:
                    doc_type = 'CreditNote'
                    break
                elif 'debitnote' in campo_lower or 'debit' in campo_lower:
                    doc_type = 'DebitNote'
                    break
            ...
            return (
                'xml',
                doc_type,
                f'urn:oasis:names:specification:ubl:schema:xsd:{doc_type}-2'
            )
            ...
            # Generic XML
            # Try to extract root from first field's xmlpath
            for field in self.fields.values():
                if field.xmlpath:
                    root = field.xmlpath.split('/')[0]
                    if root:
                        return ('xml', root, '')
