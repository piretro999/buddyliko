        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                field_data = {
                    ...
                    'business_term': row.get('business_term', '').strip(),
                    'spiegazione': row.get('spiegazione', '').strip(),
                    'obbligatorio': row.get('obbligatorio', 'NO').strip(),
                    'numerosità': row.get('numerosità', '0..1').strip(),
                    'condizionalità': row.get('condizionalità', '').strip(),
                }
                
                if is_output:
                    field_data['calcolo'] = row.get('calcolo', '').strip()
