...
class SQLiteStorage(StorageInterface):
    ...
    def save_user(self, user: Dict) -> str:
        ...
        cursor.execute('''
            INSERT INTO users (email, password_hash, name, auth_provider, auth_provider_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            user['email'],
...
class PostgreSQLStorage(StorageInterface):
    ...
    # Similar implementations for projects and users...
    # (Omitted for brevity - same pattern as SQLite)
...
class StorageFactory:
    ...
    def get_storage(config: Optional[Dict] = None) -> StorageInterface:
        ...
        if config.get('postgresql'):
            try:
                return PostgreSQLStorage(config['postgresql'])
